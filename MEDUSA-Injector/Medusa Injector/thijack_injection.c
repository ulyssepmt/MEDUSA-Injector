...
BOOL LoadShellcode(/*PVOID Loadermemory, */wchar_t* pdll, DWORD pID) {
	...
	wchar_t* className = GetWindowClassNameFromPID(pID); 
	...
	if (hProcess != INVALID_HANDLE_VALUE) {
		...
		if (allocDLLContext != 0) {
			...
			if (!SendMessageW(hwnd, WM_SYSCOMMAND, SC_RESTORE, 0)) {
		...
		if (allocDLLShellcode != 0) {
			...
			if (PrepareShellcode(allocDLLPath, allocDLLContext)) {
				...
				if (SuspendThread(hHijackThread) == (DWORD)-1) {
					...
					LogError(L" - (HIJACK) Failed to suspend the thread", GetLastError());
					return FALSE;
					}

				if (!GetThreadContext(hHijackThread, &cThread)) {

					LogError(L" - (HIJACK) Failed to get thread context", GetLastError());
				...
				if (writePathDAddress == NULL) {
					LogError(L" - (HIJACK) Failed to write memory in process (dll)", GetLastError());
					return FALSE;
					}

				BOOL writeContextAddress = WriteProcessMemory(hProcess, allocDLLContext, &cThread, sizeof(CONTEXT), 0);
				if (writeContextAddress == NULL) {
					LogError(L" - (HIJACK) Failed to write memory in process (Thread CONTEXT)", GetLastError());
					return FALSE;
					}
				...
				LogError(L" - (HIJACK) Failed to write memory in process (Shellcode)", GetLastError());
				return FALSE;
					}
				...
				if (!SetThreadContext(hHijackThread, &cThread)) {
					LogError(L" - (HIJACK) Failed to set thread context", GetLastError());
					return FALSE;
					}
				if (ResumeThread(hHijackThread) == (DWORD)-1) {
					LogError(L" - (HIJACK) Failed to resume thread", GetLastError());
					return FALSE;
					}
				...
				PostThreadMessageW(hHijackThread, WM_USER + 1, 0, 0); 
				...
				LogError(L" - (HIJACK) Can't allocate memory for shellcode", GetLastError());
				return FALSE;
					}
				}
				else {
					LogError(L" - (HIJACK) Can't allocate memory for Thread CONTEXT", GetLastError());
					return FALSE;
					}
				}
				else {
					LogError(L" - (HIJACK) Can't allocate memory for dll", GetLastError());
					return FALSE;
				}

			} else {
				LogError(L" - (HIJACK) Failed to open targeted process", GetLastError());
				return FALSE;
			}
		}
